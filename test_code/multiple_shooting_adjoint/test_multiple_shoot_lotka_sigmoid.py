...
time_span = np.linspace(0, 80.0, 800)  # 20 orbital periods and 500 points
...
class DCMFunc(nn.Module):
    ...
    def forward(self, t, y):
        out = y * 0.0
        out[0] = self.alpha * y[0] - self.beta * torch.sigmoid( y[1]) * y[0] * y[1]
        out[1] = self.delta * torch.sigmoid(y[0]) * y[1] * y[0]- self.gamma * y[1]
        return out
...
def load_time_series(index): # time_points x ROIs
    ...
    def fmri_evolve(w, t):
        x , y = w[0], w[1]
        out = w * 0.0
        out[0] = alpha * x - beta * sigmoid( y) * x * y
        out[1] = delta * sigmoid(x ) * y * x  - gamma * y
        return out
